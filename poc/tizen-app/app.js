...
var SORTS = {
  recent: function (a, b) { return b.created_ts - a.created_ts; },
  old: function (a, b) { return a.created_ts - b.created_ts; },
...
function renderDownloads() {
  ...
  var tasks = state.tasks.slice().sort(SORTS[state.dlSort]);
  $('dl-count').textContent = tasks.length + ' élément(s)';
  $('downloads-list').innerHTML = tasks.map(function (t) {
    var pct = (t.rx_pct || 0) / 100;
    var active = t.status === 'downloading';
    var seen = watchedBadge(t);
    var meta = (STATUS[t.status] || t.status) + ' · ajouté ' + fmtDate(t.created_ts)
      + (active ? ' · ' + (t.rx_rate / 1e6).toFixed(1) + ' Mo/s · reste ' + fmtEta(t.eta) : '')
      + (isPlayable(t) ? ' · ▶ OK pour regarder' : '');
    return '<div class="item" data-f tabindex="-1" id="dl-' + t.id + '" data-id="' + t.id + '">'
      + '<span class="icon">' + (ICON[t.status] || '•') + '</span>'
      + '<div class="main"><div class="title">' + esc(label(t.name)) + (seen ? ' <span class="seen">' + seen + '</span>' : '') + '</div>'
      + '<div class="meta">' + esc(meta) + '</div></div>'
      + '<span class="size">' + gb(t.size) + '</span>'
      + '<div class="progress' + (active ? ' active' : '') + '"><div style="width:' + pct.toFixed(1) + '%"></div></div>'
      + '<span class="pct">' + pct.toFixed(0) + ' %</span></div>';
  }).join('') || '<div class="empty">Aucun téléchargement.</div>';
...
async function onTaskClick(id) {
  ...
  try {
    var files = (await fbx('/downloads/' + id + '/files')).filter(function (f) { return /^video\//.test(f.mimetype || '') && !/sample/i.test(f.name); });
    files.sort(function (a, b) { return a.name.localeCompare(b.name, 'fr', { numeric: true }); });
    if (!files.length) { toast('Aucune vidéo dans ce téléchargement.'); return; }
    setVideoCount(t, files.length);
    if (files.length === 1) { play(files[0], 'downloads', t); return; }
    state.filesTask = { task: t, files: files };
    renderFiles();
    showFiles();
  } catch (e) {
    toast('Impossible de lister les fichiers : ' + e.message, true);
  }
}
